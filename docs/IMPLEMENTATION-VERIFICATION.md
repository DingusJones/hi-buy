...
The complete test journey is implemented and DOM-tested: overview → separate asset workspace → source/chart/table → literal-text note + watchlist → manual paper intent → later replay bar → modeled fill → isolated journal. Additional code provides technical indicators, scoring explanations, candidate/board selection policies, news validation, rights-guarded adapter contracts, local permitted-replay inspection, data-health surfaces, backup restore and replay alerts. See README for precise implemented/unavailable distinctions.
...
| `npm test` | 0 | **47 tests across 6 files**: formulas, confirmation lag, prefix invariance, rights, ranking boundaries, news, importer, paper ledger, backup and React DOM journey |
| `npm run fixtures:verify` | 0 | SHA-256 matches for both 440-bar vector series and fixture module |
| `npm run build` | 0 | Vite production bundle; 101 modules; approximately 360 kB JS / 113 kB gzip, 9.7 kB CSS |
| `npm run test:e2e` | 0 | **7 tests passed**: complete research/paper journey; independent boards/section absence states; four viewport runs (360/390/768/1440); export/reset/restore |
...
Playwright contains seven tests: complete research/paper journey; independent boards/section absence states; four viewport runs (360/390/768/1440); export/reset/restore. The configured preview runs on loopback. No environment policy bypass was attempted.

## Files added

- App/runtime: `src/bootstrap.tsx`, `src/main.tsx`, `src/ReplayView.tsx`, `src/style.css`, `src/vite-env.d.ts`.
- Domain/data: `src/domain.ts`, `src/data.ts`, `src/paper.ts`, `src/replay.ts`.
- Tests: `tests/domain.test.ts`, `tests/advanced.test.ts`, `tests/policies.test.ts`, `tests/paper.test.ts`, `tests/replay.test.ts`, `tests/ui.test.ts`, `tests/e2e/app.spec.ts`.
- Setup/build: `.gitignore`, `index.html`, `package.json`, `package-lock.json`, `tsconfig.json`, `vite.config.ts`, `playwright.config.ts`, `scripts/fixtures.mjs`.
- Documentation: `README.md`, `docs/FIXTURE-MANIFEST.json`, `docs/THIRD-PARTY-NOTICES.md`, this verification record.
...
1. **Market inputs and rights:** no approved real-data bundle, current prices/headlines, consolidated feed, curated 60/40 universe, complete action/calendar/benchmark data, fundamentals or ETF holdings. Boards/candidates/news have honest vacancies. Crypto and commodity default numeric values remain unavailable. Import inspection supports genuine permitted observations supplied locally, but is not live acceptance.
2. **Infrastructure not implemented:** Next.js, ECharts, PostgreSQL/migrations, durable worker/outbox, owner authentication, server API/rights enforcement, remote TLS, rate budgets, encrypted backups and an operational restore drill. The environment constraint prompted a Vite/SVG/localStorage demo; this is a documented stack deviation, not a claim that the planned architecture is complete.
...
4. **Release evidence:** browser E2E/manual accessibility, clean online dependency verification, hardware performance/uptime measurements, point-in-time evaluation and the minimum 20-session operational pilot remain outstanding.
